...
def old_run_segmentation_timestep(filepath: Path, trackspath: Path, maskspath: Path, coords: dict, parameters: dict[str, dict]) -> dict[str, pd.DataFrame]:
    meta = find_model_metadata(filepath)
    grid_level = meta['grid_level']
    dxy = find_dxy_from_grid_level(grid_level)
...
def run_segmentation_timestep(filepath: Path, trackspath: Path, maskspath: Path, coords: dict, parameters: dict[str, dict]) -> dict[str, pd.DataFrame]:
    ...
    """# take off top level if this is from WRF where extra staggered level 
    # TODO: fix staggering/interpolation so we don't need this
    if 'z' in data.dims and 'z_stag' in data.dims:
        min_levels = min(data.sizes['z'], data.sizes['z_stag'])
        data = data.isel(z=slice(0, min_levels), z_stag=slice(0, min_levels))"""
